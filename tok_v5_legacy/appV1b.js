...
// -- bar chart vis  -- // 
import ForceDirectComponent   from './ForceDirectComponent_blob'; // v2 working v3 working v5 working v6 working
...
const App = () => {
    ...
    //console.log ('row data  = ', rowsDataset)
  ...
  // -- selected item -- 
    ...
    //console.log ('all makers = ', base_nodes)
    //console.log ('rows data = ', rowData) // -- rows data set contains the results of the selection.. 
    //console.log ('town groups = ', townGroups)

    let allselected = rowData[0].makers;  // the root (inital)
    let flowselected = rowData[rowData.length-1].makers; // the paths (narrow)
    ...
    // -- dynamically generate rows -- // 
     const renderBarComponents = () => {
      ...
      console.log ("render bar components ...")
      //console.log ('sort bar data ', rowData);
      //console.log ('org data ', rowsDataset)
      sortBarData( ); // sort bars into different blocks.. 
      ...
      return rowData.map((rowData, index) => {
          ...
          return (<BarsComponent key={index} data={sourceData}index={index} onData={handleBarData} testFunction={handleRowData}/>)
      })
    ...
    // how to sort query (rows)
    // -- a function called from within the bars -- // 
    function handleRowData (data, rowId, groupId) { 
      // 
      console.log ("row = ", rowId, "group = ", groupId, "row data = ", rowData)

      let clickedRow = rowData[rowId]
      let sourceData = clickedRow.query.att === 'towns' ? townGroups : guildGroups; // 0 = towns 1 = guilds
      let foundMakers = data
      let value = sourceData[groupId].name;
      // // what is the value of the clicked items.. 

      // console.log ('clicked row = ', rowIndex)
      // -- every group has -- //
      // console.log ('show row data', rowData); // source 
      // console.log ('row groups  = ', sourceData) // the source data in the row
      // console.log ('clicked row = ', clickedRow)
      // console.log ('clicked group = ', groupId)
      // console.log ('clicked row = ', clickedRow) // the id of row selected 
      console.log ('found makers = ', foundMakers)
      console.log ('value = ', value); // get the id of the clicked group 


      /// - Q: does selection match what is already in row data 
      console.log ('row data ', rowData[rowId])
      // does the selected group match any in the query values.. 
      let match = rowData[rowId].query.value.includes(value) ? true : false; 
      console.log ("match = ", match)

      // if match = false. // add to selection (rowData)
      // if match = true. // remove from selection (rowData)

      if (match == false) { 
        let temp = [...rowData]
        temp[rowId].query.value.push (value) 
        temp[rowId].makers.push (...foundMakers)
        setRowData (temp)
        //sortBarData( )
        //renderBarComponents()
      }
    ...
    // -- update bar data... 
    // -- this is a key function -- // this needs to be called whenever the rowdata is updated..
    function sortBarData ( ) { 
      // for each element in the query - sort the bars ... ///
      console.log ("row data to sort = ", rowData)
      ...
      rowData.map((data, index) => {
              ...
              let sourceData = data.query.att === 'towns' ? townGroups : guildGroups;
              ...
              sourceData.forEach ((sourceGroup, i) => { 
                  //console.log ('source group ', sourceGroup, ': ', index)
                  ...
                  flow = nodes_to_filter.filter (n =>data.makers.includes(n)) 
                  // -- this assumes that the filtering has happened before -- // 

                  // -- 2. POSSIBLE PATHS : prev row selection but NOT in this row selection -- Possible Pathways
                  if (index > 0) {
                      paths = nodes_to_filter.filter (n =>  rowData[index-1].makers.includes(n)
                                                                    && !data.makers.includes(n)) 
                  }
    ...
    // -- do something when rowdata is updated.. 
    useEffect(() => {
      sortBarData( );
      filterTownGroups()///!!! ///
      filterGuildGroups( )
  ...
  const handleBarData = (data) => { 
    ...
    // ----- JSX ---- // 
  return (
      <div className="wrapper"style={{ backgroundColor: '#4d4d4d', width : '100%', height :'800px' }}>
        {/*<h3>ToK into React</h3>*/}
            ...
            <div className="vis-container" style={{position: 'absolute', left: '0px', top: '50px', transform: 'scale(0.5)', transformOrigin: '0% 0%' }}>
              {/*<VisualizationComponent data={data} />*/}
              <svg width={3000} height={1800} transform="translate(0, 0) scale(1)">
